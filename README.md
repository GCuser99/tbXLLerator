...
- **Complete Wiki Documentation** — See the tBXLLerator [Wiki](https://github.com/GCuser99/tbXLLerator/Wiki) for help on understanding and writing your first XLL UDF.
...
Each UDF call allocates an independent heap XLOPER12 (`AllocResultToCaller`). Excel calls `xlAutoFree12` to free memory when done. This is compatible with `ThreadSafe = True` registration that enables concurrent multi-core recalculation.
```vba
[DllExport]
...
| `HelpTopic` | `String` | `NullString` | Help redirect when "Help on this function" is clicked in Wizard |
...
| `ReturnType` | `RegDataTypes` | `rdtXLOPER12U` | Function return type |
...
3. Clone or download Jon Johnson's latest [ExcelSDK.twin](https://github.com/fafalone/TBXLLUDF) module and import to your twinBASIC project
