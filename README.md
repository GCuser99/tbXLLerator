...
My goal in writing this wrapper was to easily define and use high-performance UDF's for large and complex spreadsheet models in a language that I am familiar with - [twinBASIC](https://twinbasic.com). Specifically, I'm using UDFs in spreadsheet models along with the (as yet unstarred! :-) [SolverWrapper](https://github.com/GCuser99/SolverWrapper) for optimization, which requires worksheet fast execution.
...
5. Build the project — twinBASIC will produce a `.dll` file in the output folder
6. Rename the `.dll` to `.xll`
